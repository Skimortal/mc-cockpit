...
(function () {
  ...
  const PAGE = (function () { try { return (typeof unsafeWindow !== 'undefined' && unsafeWindow) || window; } catch (e) { return window; } })();
  ...
  // ExtJS-Grid: Zeile programmatisch über die Ext-API auswählen (synthetische Klicks ignoriert Ext).
  function extReachable() { try { return !!(PAGE.Ext && PAGE.Ext.ComponentQuery); } catch (e) { return false; } }
  function extSelectRow(po) {
    const Ext = PAGE.Ext;
    if (!Ext || !Ext.ComponentQuery) return { ok: false, why: 'Ext nicht erreichbar' };
    po = String(po);
    let views = [];
    try { views = Ext.ComponentQuery.query('tableview').concat(Ext.ComponentQuery.query('gridview')); } catch (e) { return { ok: false, why: 'query: ' + e.message }; }
    for (const view of views) {
      let store; try { store = view.getStore && view.getStore(); } catch (e) {}
      if (!store || !store.getCount) continue;
      let rec = null;
      try {
        const idx = store.findBy(function (r) { const d = (r && r.data) || {}; for (const k in d) { if (String(d[k]) === po) return true; } return false; });
        if (idx >= 0) rec = store.getAt(idx);
      } catch (e) {}
      if (!rec) continue;
      try {
        const sm = (view.ownerGrid && view.ownerGrid.getSelectionModel && view.ownerGrid.getSelectionModel())
      ...
      } catch (e) { return { ok: false, why: 'select: ' + e.message }; }
    }
    return { ok: false, why: 'PO nicht im Store gefunden' };
  }
